...
class App(Tk):
	...
	def setInfo(self):
		...
		#get user's height
		Label(self, text = "Height (in ft)").grid(row = 1, column = 0)
		...
		Label(self, text = "  Height (in inches)").grid(row = 1, column = 3)
		self.txtHeightIn = Entry(self)
		self.txtHeightIn.grid(row = 1, column = 4)
		self.txtHeightIn.insert(0, "0")

		#get user's weight
		Label(self, text = "Weight (in lbs)").grid(row = 2, column = 0)
	...
	def calcBMI(self):
		...
		if bmi < 18.5:
			...
			self.lblBMIStatus["text"] = "Obese Category-1"
		elif bmi>35 and bmi< 39.5:
			self.lblBMIStatus["text"]="Obese Category -2"
		else:
			self.lblBMIStatus["text"]="obese Category-3"
